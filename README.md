...
2. `supabase/migrations/` 配下のSQLを **番号順に** SQL Editorで実行
   ...
   - `013_daily_staffing_targets.sql`(日次個別の目標稼働人員数 + 「当日シフト状況」画面用関数)
...
目標稼働人員数はホテルのチェックアウト状況等で日々変わるとのご相談を受け、
シフト時間帯の「固定のデフォルト値」に加えて、**日付ごとに個別の目標人数を
設定できる**ようにしました。

### 実装内容

- `daily_staffing_targets`テーブル(シフト時間帯×日付ごとの目標人数)を追加。
  `shift_templates.target_headcount`は「日次で個別設定がない場合に使う
  デフォルト値」という位置付けに変わりました。
- **新画面`/shifts/staffing`(当日シフト状況)**:ホテル・日付(前日/翌日
  ボタン付き)を指定し、その日のシフト時間帯ごとに目標人数を直接入力・保存でき、
  承認済み人数との差分(不足/余裕)がその場で確認できます。
- `/shifts/calendar`の月次「人員充足状況」テーブルも、日次の個別設定があれば
  そちらを優先するよう更新し、「当日シフト状況」画面へのリンクを追加しました。
- `hotel_id`はDBのトリガーで`shift_template_id`から自動的に補完しており、
  クライアントから送られた値は使いません(整合性の不一致を防ぐため)。
- 当日シフト状況の閲覧・入力はsuper_admin/manager両方が行えます
  (現場での日次調整を想定しているため)。過去日・未来日どちらでも
  設定できます(例:来週の予約状況を見て前もって設定する、という使い方も可能)。
